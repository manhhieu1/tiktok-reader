  ...
  if (uniqueId !== "") {
    ...
    connection
      ...
      .then((state) => {
        ...
        $("#roomInfo").html(
          `
          <img src="${state.roomInfo.owner.avatar_large.url_list[0]}" alt="" style="object-fit: cover; width: 150px; height: 150px; border-radius:75px" />
          <p>${state.roomInfo.owner.nickname} - ${state.roomInfo.owner.display_id}</p>
          <p>${state.roomInfo.owner.follow_info.follower_count} - ${state.roomInfo.owner.follow_info.following_count}</p>
        `
        );
